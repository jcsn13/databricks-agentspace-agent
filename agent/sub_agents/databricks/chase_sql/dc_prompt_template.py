...
* **Pergunta Principal (contagem de restaurantes):**
   * **SQL:** SELECT COUNT(`T1`.`id_restaurant`) FROM `{DATABRICKS_CATALOG}.restaurant.generalinfo` AS `T1` INNER JOIN `{DATABRICKS_CATALOG}.restaurant.location` AS `T2` ON `T1`.`id_restaurant` = `T2`.`id_restaurant` WHERE `T1`.`food_type` = 'thai' AND `T2`.`street_name` = 'san pablo ave' AND `T2`.`city` = 'albany'
