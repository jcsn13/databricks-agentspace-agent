...
def initial_databricks_nl2sql(
    ...
    ddl_schema = tool_context.state["database_settings"]["databricks_ddl_schema"]
    catalog = tool_context.state["database_settings"]["databricks_catalog"]
    schema = tool_context.state["database_settings"]["databricks_schema"]
    transpile_to_databricks = tool_context.state["database_settings"][
        "transpile_to_databricks"
    ]
    ...
    # If postprocessing of the SQL to transpile it to Databricks is required,
    # then do it here.
    if transpile_to_databricks:
        ...
        responses: str = translator.translate(
            responses, ddl_schema=ddl_schema, db=schema, catalog=catalog
        )
