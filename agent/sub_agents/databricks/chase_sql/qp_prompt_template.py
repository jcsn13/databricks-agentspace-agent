...
Agora você precisa gerar uma consulta Databricks SQL ou Spark SQL dadas as informações do banco de dados, uma pergunta e algumas informações adicionais.
...
2. **Agregação (MAX/MIN):**
   - Certifique-se de que os `JOIN`s sejam concluídos antes de aplicar `MAX()` ou `MIN()`. O Databricks SQL suporta sintaxe padrão para funções de agregação, então use `MAX()` e `MIN()` conforme necessário após as operações `JOIN`.

3. **ORDER BY com Valores Distintos:**
   - No Databricks SQL, `GROUP BY <coluna>` pode ser usado antes de `ORDER BY <coluna> ASC|DESC` para obter valores distintos e ordená-los.
...
5. **Cláusulas FROM/JOIN:**
   - Inclua apenas tabelas essenciais para a consulta. O Databricks SQL suporta tipos de `JOIN` como `INNER JOIN`, `LEFT JOIN` e `RIGHT JOIN`, então use-os com base nas relações necessárias.
...
10. **Concatenação de Strings:**
    - O Databricks SQL suporta tanto `CONCAT()` quanto `||` para concatenação de strings. Use `CONCAT(coluna1, ' ', coluna2)` ou `coluna1 || ' ' || coluna2` para concatenação.
...
12. **Apenas Funções Databricks SQL:**
    - Use funções disponíveis no Databricks SQL/Spark SQL. Use funções como `DATE_FORMAT`, `YEAR`, `MONTH`, `DAY` para manipulação de datas.

13. **Processamento de Datas:**
    - O Databricks SQL suporta `YEAR(coluna_data)` para extrair o ano ou `DATE_FORMAT(coluna_data, 'yyyy')`. Use funções de data como `DATE_SUB`, `DATE_ADD` e `DATEDIFF` para manipulação de datas.

14. **Nomes de tabela e referência:**
    - Conforme exigido pelo Databricks, sempre use o nome completo da tabela no formato catalog.schema.table na instrução SQL. Por exemplo, "SELECT * FROM catalog.schema.tabela_a", não apenas "SELECT * FROM tabela_a"
...
CREATE TABLE {DATABRICKS_CATALOG}.restaurant.generalinfo
(
 id_restaurant INT64,
...
CREATE TABLE {DATABRICKS_CATALOG}.restaurant.location
(
 id_restaurant INT64,
...
SELECT COUNT(T1.id_restaurant)
 FROM {DATABRICKS_CATALOG}.restaurant.generalinfo AS T1
 INNER JOIN {DATABRICKS_CATALOG}.restaurant.location AS T2 ON T1.id_restaurant = T2.id_restaurant
...
CREATE TABLE {DATABRICKS_CATALOG}.financial.account (
   account_id INT64,
   district_id INT64,
   frequency STRING,
   date DATE,
);
CREATE TABLE {DATABRICKS_CATALOG}.financial.client (
   client_id INT64,
   gender STRING,
   birth_date DATE,
   district_id INT64,
);
CREATE TABLE {DATABRICKS_CATALOG}.financial.district (
   district_id INT64,
...
```sql
SELECT `T1`.`gender`
 FROM `{DATABRICKS_CATALOG}.financial.client` AS `T1`
 INNER JOIN `{DATABRICKS_CATALOG}.financial.district` AS `T2`
...
**************************
【Informações do Banco de Dados】
CREATE TABLE {DATABRICKS_CATALOG}.olympics.games
(
 id INT64,
...
CREATE TABLE {DATABRICKS_CATALOG}.olympics.games_city
(
 games_id INT64,
 city_id INT64 OPTIONS(description="o id da cidade que sediou o jogo Mapeia para city(id)"),
);

CREATE TABLE {DATABRICKS_CATALOG}.olympics.city
(
 id INT64,
 city_name STRING,
);
...
```sql
SELECT COUNT(T3.id)
 FROM {DATABRICKS_CATALOG}.olympics.games_city AS T1
 INNER JOIN {DATABRICKS_CATALOG}.olympics.city AS T2 ON T1.city_id = T2.id
 INNER JOIN {DATABRICKS_CATALOG}.olympics.games AS T3 ON T1.games_id = T3.id
...
**************************
【Informações do Banco de Dados】
CREATE TABLE {DATABRICKS_CATALOG}.retails.employees (
   employee_id INT64,
   department_id INT64,
   salary INT64,
);
...
**Consulta SQL Otimizada Final:**
```sql
SELECT COUNT(*) FROM {DATABRICKS_CATALOG}.retails.employees WHERE salary > 100000;
```
